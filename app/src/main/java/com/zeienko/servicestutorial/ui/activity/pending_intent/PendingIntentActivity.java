...
public class PendingIntentActivity extends AppCompatActivity {
    ...
    public void onClickStart(View v) {
        ...
        pi = createPendingResult(TASK1_CODE, data, 0);
        // Создаем Intent для вызова сервиса, кладем туда параметр времени
        // и созданный PendingIntent
        intent = new Intent(this, PendingIntentService.class)
                .putExtra(PARAM_TIME, 7)
                .putExtra(PARAM_PINTENT, pi);
        // стартуем сервис
        startService(intent);

        pi = createPendingResult(TASK2_CODE, data, 0);
        intent = new Intent(this, PendingIntentService.class)
                .putExtra(PARAM_TIME, 4)
                .putExtra(PARAM_PINTENT, pi);
        startService(intent);

        pi = createPendingResult(TASK3_CODE, data, 0);
        intent = new Intent(this, PendingIntentService.class)
